...
client.on("message", (message) => {
    ...
    if(command == "!hangman") {
        if(message.guild === null) {
          message.channel.send("Sorry, but you cannot play hangman with me.. :(");
          return;
        }
        const embed = new Discord.RichEmbed()
            .setTitle("Hangman")
            .setDescription("You're playing hangman on **"+message.guild.name+"** server.")
            .addField("Please come up with a word and write it to me...", "If you don't want to play hangman, type `cancel`")
            .setColor("ORANGE")
            .setTimestamp();
        client.users.get(message.author.id).send(embed).then(startMsg => {

            const collector = new Discord.MessageCollector(startMsg.channel, m => m.author.id == message.author.id, { time: 1000000 });
            collector.on('collect', msg => {

                //Cancel
                if(msg.content.latinize().toLowerCase() == "cancel") {
                    msg.channel.send("Okay, I've canceled this game!");
                    collector.stop();
                    return;
                }

                //Two or more spaces
                if((msg.content.match(new RegExp(" ", "g")) || []).length > 1) {
                    msg.channel.send("Sorry, but the word cannot contain 2 or more spaces, please think of another word!");
                    return;
                }

                //Longer than 20 characters
                if(msg.content.length > 20) {
                    msg.channel.send("Sorry, but the word cannot be longer than 20 characters, please think of another word!");
                    return;
                }

                //Shorter than 3 characters
                if(msg.content.length < 3) {
                    msg.channel.send("Sorry, but the word must be at least 3 characters long, please think of another word!");
                    return;
                }

                //Other characters
                if(hasOnlyLetters(msg.content) == false) {
                    msg.channel.send("Sorry, but the word must contain only letters of the latin alphabet, please think of another word!");
                    return;
                }
                ...
                //The given word
                var word = msg.content.toUpperCase();

                msg.channel.send("I've successfully created a hangman game with the word **"+word+"** on **"+message.guild.name+"** server.");

                //Check if message contains space
                if(msg.content.includes(" ")) {
                    var msgArgs = msg.content.split(" ");
                    var emojiDescription = "⏹".repeat(msgArgs[0].length)+" "+"⏹".repeat(msgArgs[1].length);
                } else {
                    var emojiDescription = "⏹".repeat(msg.content.length);
                }
        ...
        function game(gameMsg, word) {

            const collector = new Discord.MessageCollector(message.channel, m => true, { time: 1000000 });
            collector.on('collect', msg => {
